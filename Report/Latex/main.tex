...
\begin{itemize}
    ...
    \item $r$ is the immediate reward for achiveing a state
\end{itemize}
...
\begin{figure}[h]
    ...
    \caption{\textbf{X} training against perfect \textbf{O} causes the drawrate to icnrease in the form of a decaying exponential}
...
\begin{figure}[h]
    ...
    \caption{\textbf{X} training against perfect \textbf{O} causes the \textbf{X} winrate to increase in the form of a decaying exponential}
    \label{fig:x_vs_random_distribution_1}
\end{figure}

\section{Variations in $\alpha$ and $\tau$}
The values of hyperparameters $\tau$ and $\alpha$  matter significantly. During the game it is best to decrease the learning rate. Initially it desired for the new infromation to rapidly change the Q-Table, but eventually a low learning rate means that one bad game will not significantly the Q-Values trained over hundreds of games in the past. Meanwhile $\tau$ should increase swithcing from exploration to exploitation. Different models for $\tau$ and $\alpha$ have been tested and two models are proposed as best candidates as seen in 
...
\end{tabular}
% 3. ADD THE CAPTION AND LABEL HERE
\caption{The theoretical Q-Table for x playing againsta  random opponent, and RL-estimated Q-Table for x playing against a random opponent give different values, because the future policy is different}
...
\section{Neural Network structure, backpropagation, cost fucntion}

By structure DQN is identical to a normal neural network. It takes in the state such as coordinates or stock prices in the input layer and forward propogates the input through the network to return the estimated Q-Values for different actions in the output layer as seen in equation \ref{eq:forward propagation}. 
...
\end{figure}
\chapter{Conclusion}
In this project I have successeded in implementing traditional Q-Learning in the context of tic-tac-toe and with help of AI implemented Deep Q-Learning in the context of two simple games. During Q-Learning in tic-tac-toe agents learn in an exponential manner against perfect and a random opponent drawing and winning more respectively. The most notable feature discovered when making agents trained on different policies play against each other, was that agents trained on the random policy would outperform agents trained on the perfect policy. The RL-estimated Q-Table was compared to a theoretical Q-Table was similar during training against a perfect opponent but was different against a random opponent due to differences in future policy. During Deep Q-Learning implementation a small numpy-based library was which could then be implemented in any context, such as the ``CartPole'' and ``LunarLander'' games developed by Open-AI.
...
All the code for this project along with the report is accessible in a public GitHub repositry. It can be accessed via this link:\\ \url{https://github.com/peterladur/Reinforcement-Learning-Research-Project}
