...
When generating the Q-Table of \textbf{X} playing against a random opponent, more states are explored, and two out of three actions which lead to a garanteed win (assuming perfect play), were given the q-value of 1 as seen in Table \ref{tab:Q-Table comparison for _______ox}. The reason why the action (2, 2) is given a q-value of 0, is likely due to exploration vs exploitation.
...
\cline{2-10}
\multicolumn{1}{c|}{} & (1, 1) & (1, 2) & (1, 3) & (2, 1) & (2, 2) & (2, 3) & (3, 1) & (3, 2) & (3, 3) \\ \hline
\makecell{Theoretical Q-Table \\(perfect opponent)}& \cellcolor{black!60}& -1 & -1 & 0 & -1& \cellcolor{black!60} & -1 & \cellcolor{black!60} & \cellcolor{black!60}\\ \hline
RL Q-Table & \cellcolor{black!60}& 
-0.87 & -0.78 & 0 & -0.94 & \cellcolor{black!60} & -0.75 & \cellcolor{black!60} & \cellcolor{black!60}\\ \hline
\end{tabular}
% 3. ADD THE CAPTION AND LABEL HERE
\caption{The theoretical and RL Q-Table for x playing against perfect opponent, give similar values for the state }
...
\cline{2-10}
\multicolumn{1}{c|}{} & (1, 1) & (1, 2) & (1, 3) & (2, 1) & (2, 2) & (2, 3) & (3, 1) & (3, 2) & (3, 3) \\ \hline
\makecell{Theoretical Q-Table \\(perfect opponent)}& 0 & 0 & 1 & 0 & 1 & 1  & 0 & \cellcolor{black!60} & \cellcolor{black!60}\\ \hline
\makecell{Theoretical Q-Table \\(random opponent)}& 0.47 & 0.47 & 0.57 & 0.32 & 0.72 & 0.42  & 0.24 & \cellcolor{black!60} & \cellcolor{black!60}\\ \hline
RL Q-Table & 0.30& 0.20 & 1.00 & 0 & 0 & 1.00 & 0 & \cellcolor{black!60} & \cellcolor{black!60}\\ \hline
\end{tabular}
% 3. ADD THE CAPTION AND LABEL HERE
\caption{The theoretical and RL Q-Table achieve }
...
The main disadvantage of traditional Q-Learning is that it only works well with a small number of discrete states. If the input is continuous, then it will no longer work, as the Q-Table would need to be infinitely large. This problem is solved by Deep Q-Learning (DQN), instead of a Q-Table, a Deep Q-Network (DQN) is implemented. While a Q-Table a state corresponds to all the Q-Values which can be taken by different actions, with a DQL uses a network to estimate the Q-Values for all actions instead.\\
...
By structure DQN is identical to a normal neural network. It takes in the state such as coordinates or stock prices in the input layer and forward propogates the input (as seen in equation \ref{eq:forward propagation}) through the network to return the estimated Q-Values in the output layer. 
...
The main difference between a Q-Table and a Q-Network is while during training a Q-Table, the Q-Values are simply updated using the Bellman equation, in a DQN weights and biases are adjusted. With a Q-Table that has converged $Q(s, a) = r + \gamma \max_{a'} Q(s', a')$ and very similarly, with a trained Deep Q-Network $Q(s, a, \theta) = r + \gamma \max_{a'} Q(s', a', \theta)$. During DQN training, an action picked is compared to the target as seen in equations \ref{eq:target} and \ref{eq:loss function}. The weights and biases $\theta$ are then adjusted through backpropogation with respect to the loss function $\mathcal{L}(\bm{\theta})$ as seen in code segment \ref{}.
...
\begin{equation}
    \mathcal{L}(\bm{\theta}) = \mathbb{E}_{(s, a, r, s')} \left[\frac{1}{2} \left( \text{target} - Q(s, a; \bm{\theta}) \right)^2 \right]
