...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        \text{\textbf{O} trained on perfect} & \makecell{\textbf{X} winrate: 57.97\% \\ drawrate: 14.5\%\\ \textbf{O} winrate:28.04\%} & \makecell{\textbf{X} winrate: 77.51\% \\ drawrate: 8.74\%\\ \textbf{O} winrate: 13.75\%} \\ \hline
        
        \text{\textbf{O} trained on random} & \makecell{\textbf{X} winrate: 29.62\% \\ drawrate: 16.24\%\\ \textbf{O} winrate: 54.14\%} & \makecell{\textbf{X} winrate: 51.12\% \\ drawrate: 11.87\%\\ \textbf{O} winrate: 37.01\%} \\ \hline
    \end{tabular}
...
The main disadvantage of traditional Q-Learning is that it only works well with a small number of discrete states. If the input is continuous, then it will no longer work, as the Q-Table would need to be infinitely large. This problem is solved by Deep Q-Learning, instead of a Q-Table, a Deep Q-Network (DQN) is implemented. The network takes in the state in the input layer and returns the q-values in the output layer.\\

I have implemented a simple python, numpy-based library which allows for an implementation of a Deep Q-Network.
\section{Neural Network structure, backpropogation, loss fucntion}

The Deep Q-Network takes in the continuous state such as coordinates or stock prices in the input layer. Then the DQN forward propogates the input through the network and returns the output values with 
...
\begin{equation}
    \mathcal{L}(\bm{\theta}) = \mathbb{E}_{(s, a, r, s') \sim \mathcal{D}} \left[ \left( r + \gamma \max_{a'} Q(s', a'; \bm{\theta}^-)- Q(s, a; \bm{\theta}) \right)^2 \right]
\end{equation}
