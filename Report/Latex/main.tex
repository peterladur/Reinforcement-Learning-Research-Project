...
The learning rate $\alpha$ determines how much does the new information replace the already known information. A learning rate of $1$ would that each time training iteration the $Q(s, a)$ is completely replaced by $(r + \gamma \cdot \max_{a}{Q(s_{t + 1}, a)})$. A learning rate of $0$ would imply that $Q(s, a)$ is not affected by training any more. The learning rate can be adjusted during training as the model becomes more stable.\\
...
As agents learn their drawrate or winrate depending on if they are playing against a perfect or random opponent increases in the form of the decaying exponential as seen in . \textbf{X} learns against optimal \textbf{O} the draws increase in the form of a decaying exponential draws \% = $e^{-k \cdot \text{games played}}$ as seen in Figure \ref{fig:x_vs_perfect_distribution_1}.  
