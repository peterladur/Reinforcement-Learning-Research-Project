...
def find_average(entries:list):
    """finds the average of a list ignoring null values"""
    if entries == None:
        return -1
...
def generate_all_possible_states():
    ...
    for turn in range(8, -1, -1):
        ...
        for combination in possible_states[turn + 1]: #goes through each "paparent combination"
                    ...
                    if result != None: #if state is valid 
                        ...
                        if result != 2: #if the game state is a determined state
                            ...
                            if validity != 2: #If the original state is terminal, then the q value for that entry is the result
                                ...
                                Q_Table[str_combination][i] = find_average(Q_Table.setdefault(combination, [])) #We pick the expected value with random play (average value)
...
def print_and_write_statistics(possible_states, Q_Table, json_filename="results/random_Q_Table.json"):
